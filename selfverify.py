...
(CLAUDE.md §1, COMPARISON.md §1.1). This is IQ: billing.total must come back
as `double precision`, and every non-zero bill must still be a whole multiple
of 250 IQD, because IQ's defining invariant is that a payable figure can be
handed over in real notes. JO's copy of this file asserts the opposite things
(`numeric`, 3-decimal scale) and must never be reconciled with this one.
"""
import json
import os
import secrets
...
# --- IQ's money model. See the module docstring. ---
MONEY_EXPECTED_TYPE = "double precision"
DENOMINATION = 250
...
def _run_checks(con):
    ...
    # IQ only: every payable figure must be handable in real notes. A restore
    # that rounded or coerced values breaks this and nothing else would notice.
    off_note = scalar(
        f"SELECT count(*) FROM billing WHERE total > 0 "
        f"AND (total::numeric % {DENOMINATION}) <> 0")
    checks.append(_check(
        f"every non-zero bill is a whole multiple of {DENOMINATION} IQD",
        off_note == 0,
        "all payable in notes" if off_note == 0 else f"{off_note} bill(s) are not",
    ))
    return checks
