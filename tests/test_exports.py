...
def billed_visit(client, db):
    ...
    client.post(f"/visits/{v_id}/billing",
                data={"billing_type": "Manual", "manual_amount": "10000"}, follow_redirects=False)
    client.post(f"/visits/{v_id}/payment",
                data={"amount": "4000", "method": "Cash"}, follow_redirects=False)
...
def completed_sale_for_receipt(client, db):
    ...
    db.execute("INSERT INTO inventory_list (id, name, category, unit, track_expiry, cost_price, "
               ...
               (inv_id, f"Receipt Item {inv_id}", "Retail", "unit", False, 1000.0, "Owned", True))
    db.execute("INSERT INTO price_list (id, name, category, cost_price, sale_price, active, "
               "linked_item_id, can_discount) VALUES (?,?,?,?,?,?,?,?)",
               (pl_id, f"Receipt Item {inv_id}", "Retail", 1000.0, 5000.0, True, inv_id, True))
    ...
    client.post("/pos/checkout", data={
        ...
        "payment_method": "Cash", "cash_received": "15000"}, follow_redirects=False)
...
def test_boarding_export_produces_a_pdf(client, db, billed_visit):
    cur = db.execute(
        ...
        (billed_visit["patient_id"], date.today().isoformat(), False, False, 0.0, 0.0, False, 20000.0))
    bid = cur.fetchone()["id"]
    db.commit()
    try:
