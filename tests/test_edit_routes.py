...
def stay(db, patient):
    cur = db.execute(
        ...
        "VALUES (?,?,?,?,?,?,?,?,?) RETURNING id",
        (patient["patient_id"], date.today().isoformat(), False, True, 0.0, 0.0, False,
         20000.0, 5000.0))
    bid = cur.fetchone()["id"]
    db.commit()
...
def _edit_stay(client, db, bid, **data):
    payload = {"entry_date": date.today().isoformat(), "dismissal_date": "",
               "price_per_day": "5000", "total": "", "room": "R1",
...
def test_a_stay_can_be_edited(client, db, stay):
    _edit_stay(client, db, stay["id"], price_per_day="6000")
    row = db.execute("SELECT * FROM boarding_sessions WHERE id=?", (stay["id"],)).fetchone()
    assert row["price_per_day"] == 6000.0


def test_a_stay_rejects_a_negative_daily_rate(client, db, stay):
    _edit_stay(client, db, stay["id"], price_per_day="-6000")
    row = db.execute("SELECT * FROM boarding_sessions WHERE id=?", (stay["id"],)).fetchone()
    assert row["price_per_day"] == 5000.0, "a rejected edit must leave the rate alone"
...
def test_a_stale_stay_edit_is_refused(client, db, stay):
    ...
    assert after == 7000.0, "the stale save must not overwrite the first"
...
def case(db, patient):
    cur = db.execute(
        ...
        (patient["patient_id"], date.today().isoformat(), False, 0.0, 0.0, 0.0))
    cid = cur.fetchone()["id"]
    db.commit()
...
def priced_service_for_case(db):
    ...
    db.execute("INSERT INTO price_list (id, name, category, cost_price, sale_price, active, can_discount) "
               "VALUES (?,?,?,?,?,?,?)",
               (pl_id, f"Case Service {pl_id}", "Service", 2000.0, 6000.0, True, True))
    db.commit()
    yield {"id": pl_id}
...
def _opex(client, month, **data):
    payload = {"month": month, "rent": "100000", "salaries": "500000",
               "utilities": "50000", "marketing": "20000", "other": "0"}
    payload.update(data)
...
def test_operating_costs_can_be_recorded(client, db, opex_snapshot):
    _opex(client, opex_snapshot, rent="120000")
...
def test_operating_costs_reject_a_negative_figure(client, db, opex_snapshot):
    ...
    _opex(client, opex_snapshot, rent="100000")
    resp = _opex(client, opex_snapshot, rent="-100000")
    assert resp.status_code != 500
    row = db.execute("SELECT * FROM monthly_opex WHERE month=?", (opex_snapshot,)).fetchone()
    if row is not None:
