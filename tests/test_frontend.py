"""
Frontend regression tests — VetClinicSystem IQ.
...
app. See the equivalent file in JO — the two are close but not identical,
because IQ has four palettes and JO has one.
"""
import re
import pathlib
...
STANDALONE_TEMPLATES = {"base.html", "_back_link.html", "_visit_fields.html",
...
def test_no_hardcoded_colours_outside_the_palette(css_no_comments):
    """JO once shipped a Settings restore button in a colour that existed
    nowhere in its palette. A hardcoded hex looks fine in the theme it was
...
def test_every_palette_defines_the_same_tokens():
    """IQ has four palette/theme combinations. A token missing from one of
    them silently inherits the previous theme's value — which is exactly how
    the dark-mode contrast bug in COMPARISON.md §12 happened. All override
    blocks must cover the same ground."""
    css_text = CSS_PATH.read_text(encoding="utf-8")
    overrides = _block_bodies(css_text, r'(?m)^html\[data-(?:theme|palette)[^{]*\{')
    sized = {sel: toks for sel, toks in overrides.items() if toks}
    assert len(sized) >= 2, "expected several palette/theme override blocks"
    reference_sel, reference = next(iter(sized.items()))
    for sel, toks in sized.items():
        assert toks == reference, (
            f"{sel} and {reference_sel} define different token sets — "
            f"missing here: {sorted(reference - toks)}, extra: {sorted(toks - reference)}")
...
def test_keyboard_focus_is_visible(css):
    ...
    what you have selected. JO shipped for months with none of it."""
...
def test_folder_browser_rows_are_styled(css):
    """Shipped unstyled in JO — a wall of undifferentiated text in the
    backup-folder picker. Cheap to lose again in a CSS tidy-up."""
