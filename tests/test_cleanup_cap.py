...
Amounts here are IQD against money.CLEANUP_CAP. JO's copy of this file uses
JOD against its own Decimal cap — the same rules, each stated in its app's own
money model, which must not be merged (COMPARISON.md §1.1).
"""
import pytest
...
def test_a_negative_write_off_is_refused():
    ...
    assert "negative" in cleanup_amount_error(-1, 0, 100000)


def test_a_write_off_over_the_cap_is_refused():
    assert "exceed" in cleanup_amount_error(CAP + 250, 0, 100000)
...
def test_the_cap_accumulates_across_submissions():
    ...
    assert cleanup_amount_error(CAP, CAP, 100000) is not None
    assert cleanup_amount_error(250, CAP - 250 + 250, 100000) is not None


def test_a_write_off_larger_than_the_balance_is_refused():
    """GUARD. Writing off more than is owed would turn a bill negative."""
    err = cleanup_amount_error(500, 0, 250)
