...
clean_date() before it can reach a query.
...
Ported from JO 2026-08-26 (divergence audit finding 7.6) and strengthened
on the way -- JO's original only saw a form variable literally named `f`,
only scanned app.py, and rejected a legitimate multi-line clean_date(...)
call. See the two control tests below: this file's own detector is tested,
because a guard that silently stops matching anything passes forever
without checking a thing (CLAUDE.md section 7.3).
"""
import re
import pathlib
...
# IQ validates every write-side date through clean_date() (app.py). JO also
# has clean_date_filter(); IQ has no such helper, so it is deliberately not
# listed here -- keep this list matching THIS app's actual code.
WRAPPERS = ("clean_date(",)
...
# written; the first four cases are ones JO's original rule missed entirely.
