...
def test_the_money_check_asserts_this_app_s_own_model(clean_backup_log, tmp_path):
    """IQ: double precision. JO's copy asserts numeric. If these two files are
    ever reconciled into one, this fails in whichever app is wrong."""
    import selfverify
    assert selfverify.MONEY_EXPECTED_TYPE == "double precision"
    assert selfverify.DENOMINATION == 250

    _log_backup(clean_backup_log, _real_dump(tmp_path))
    result = selfverify.verify_latest_backup(clean_backup_log)
    names = {c["name"] for c in result["checks"]}
    assert "billing.total is double precision" in names
    assert "every non-zero bill is a whole multiple of 250 IQD" in names
...
def test_a_backup_whose_money_column_is_the_wrong_type_fails(clean_backup_log, tmp_path):
    ...
    NUMERIC is JO's money model, and correct there — restored into IQ it is
    wrong, and it is the kind of wrong that looks fine today and loses money
    on the next write. Nothing else in this layer would notice.
    """
    import selfverify
    path = _dump_of_scratch_db(tmp_path, "NUMERIC(12,3)")
    _log_backup(clean_backup_log, path)

    before = _throwaway_databases()
    result = selfverify.verify_latest_backup(clean_backup_log)
    assert result["result"] == "fail", (
        "a backup whose billing.total is numeric must not verify in IQ: %r"
        % result["detail"]
    )
    failed = {c["name"] for c in result["checks"] if not c["ok"]}
    assert "billing.total is double precision" in failed, (
...
def test_the_control_the_same_backup_with_the_right_money_type_passes(
    ...
    import selfverify
    path = _dump_of_scratch_db(tmp_path, "DOUBLE PRECISION", name="rightmoney.dump")
    _log_backup(clean_backup_log, path)

    result = selfverify.verify_latest_backup(clean_backup_log)
    assert result["result"] == "pass", (
        "the identical database with IQ's own money type must verify: %r"
        % result["detail"]
    )
