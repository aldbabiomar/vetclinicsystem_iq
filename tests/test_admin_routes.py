...
def test_the_discount_cap_is_what_actually_limits_a_discount(client, db):
    """Ties the role setting to the money rule it exists to enforce, rather
    than trusting that the column is read anywhere."""
    import app as app_module
    for cap, percent, should_pass in ((10, 5, True), (10, 10, True),
                                      (10, 25, False), (0, 1, False)):
        err = app_module.discount_percent_error(percent, cap)
        assert (err is None) == should_pass, (
            f"cap={cap} percent={percent}: expected {'accepted' if should_pass else 'refused'}")
