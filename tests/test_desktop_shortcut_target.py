...
CHANGELOG 1.6.1): resolving the launcher relative to whichever copy of the
code happens to be running, which pins the shortcut to one frozen
vetclinicsystemiq-releases/app_vX.Y.Z/ snapshot. The next update flips
...
The only correct target is the supervisor launcher in vetclinicsystemiq-data/,
...
def _managed_install(root):
    ...
    data_dir = os.path.join(root, "vetclinicsystemiq-data")
    release_dir = os.path.join(root, "vetclinicsystemiq-releases", "app_v9.9.9")
...
def sandbox(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        os.makedirs(desktop)
        bundle = os.path.join(desktop, "VetClinicSystem IQ.app")
        monkeypatch.setattr(desktop_shortcut, "_macos_bundle_path", lambda: bundle)
        monkeypatch.delenv("VETCLINICSYSTEMIQ_DATA_DIR", raising=False)
...
def test_running_from_a_release_folder_still_targets_the_data_dir(sandbox, monkeypatch):
    ...
    assert "vetclinicsystemiq-releases" not in line, (
...
def test_running_from_the_original_checkout_also_targets_the_data_dir(sandbox, monkeypatch):
    ...
    checkout = os.path.join(sandbox["root"], "vetclinicsystem_iq-main")
...
def test_generated_script_quotes_paths_with_spaces(sandbox, monkeypatch):
    """Every real install path here contains a space ("Start VetClinicSystem.command"), so an unquoted interpolation would word-split at runtime."""
    monkeypatch.setattr(autostart, "BASE_DIR", sandbox["release_dir"])
