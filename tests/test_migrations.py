...
runs `apply_schema()` then `apply_incremental_migrations()` against the
live clinic database, with `check=True` — so anything that raises there
...
def _upgrade(url):
    ...
    return subprocess.run(
        [sys.executable, "-c",
         "import setup; setup.apply_schema(); setup.apply_incremental_migrations()"],
