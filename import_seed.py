...
def main():
    ...
    # Roles/permissions must exist before we can point the seeded admin
    # user at Admin's role_id.
    auth.seed_default_roles_and_permissions(cur)
    ...
    for row in data["billing"]:
        ...
        # per-row figure available here, coerced safely rather than
        # assumed numeric.
        try:
            amount = float(row[1]) if row[1] not in (None, "") else 0
