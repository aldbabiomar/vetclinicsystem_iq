...
ADMIN_ONLY_TODAY = {
    ...
    # relationship — kept admin-only from day one rather than defaulting
    # it open like everything else non-admin here, unlike view/receiving/
    # shrinkage logging, which are day-to-day front-desk/stock work.
...
def no_vet_role_configured(db):
    ...
    all render with no options (not a crash — see logic.vet_users() —
    but a clinic can't actually schedule anything). Callers use this to
    surface a loud warning right when an admin's role edit/delete would
    cause it, instead of letting it be a support ticket someone files
    later after wondering why the vet dropdown is empty everywhere.
    """
...
def is_system_admin():
    """True if the current session belongs to the (locked) Admin role."""
...
def log_change(db, table_name, record_id, action, changes=None):
    ...
    reach its own db.commit() afterward — see app.py call sites, which
    all call log_change() before their explicit commit.
    """
    uid = session.get("user_id")
