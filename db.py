...
def connect():
    ...
    scripts (setup.py, import_seed.py), the app's background scheduler
    (nightly backup), and the final backup taken on shutdown. Those are
    low-frequency, long-or-uncertain-duration operations (a backup or a
    restore can run for minutes) that don't belong sharing a small pool
    with request traffic, so they keep opening their own short-lived
...
# get_db()/close_db()) and for the insights dashboard's parallel report
# queries. Previously every request opened a brand-new PostgreSQL
# connection with no limit; under a burst of LAN traffic (multiple
# clinic devices, each with Waitress's 8 worker threads) that could pile
# up faster than Postgres's own max_connections, degrading into
...
def _pool_settings():
    ...
    8 worker threads plus a few background-job connections (insights,
    backups) without opening the door to unbounded growth."""
