...
database and its own clinic name/color palette set in Settings).
...
- Barcodes: generate one for any item that doesn't have a manufacturer
  barcode, print a single label, or bulk-print labels for many items at
  once.
...
**Settings** — clinic name, location, and color palette (the whole UI is
themeable per install); numeric thresholds (audit-overdue days, expiry-soon
days, appointment slot length); nightly backup folder, time, and retention;
and, on an install that's opted into the versioned-release layout, **in-app
updates** — check for a new version, apply it (automatic pre-update backup,
progress shown step by step), or roll back to the previous release with one
click if something's wrong.
...
There are 698 tests in 37 files, and they are not all the same kind. Each
...
`scripts/isolated_test_env.sh up iq` builds that database and a venv with
