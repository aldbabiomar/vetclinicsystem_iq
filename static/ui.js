...
(function () {
  ...
  // for every tabindex="0" role="link" table row (patients, owners,
  // visits, inpatient, owner_detail, patient_detail). Rows already carry
  // data-row-href (added alongside this script) instead of inline onclick,
  // so a press-and-drag scroll on a touchscreen doesn't also trigger the
  // row underneath (§10).
  // ---------------------------------------------------------------
  function initRowNav() {
  ...
  // ---------------------------------------------------------------
  // 1.4 — Sidebar group collapsing (Inventory, Sales & Billing, Admin),
  // remembers last open/closed state per person via localStorage. Dashboard,
  // Patients & Visits, and Inpatient stay always-expanded (no toggle
  // rendered for those groups). Every link stays exactly where it is —
  // this only adds a fold.
  // ---------------------------------------------------------------
  ...
  // every .modal-overlay in the app (appointments' New Appointment,
  // price-list edit modal, the unsaved-changes modal). A modal is opened
  // via data-open-modal="#id" on the trigger, closed via data-close-modal
  // or a backdrop click.
  // ---------------------------------------------------------------
  function initModals() {
  ...
  // ---------------------------------------------------------------
  // htmx wiring for the partial-update layer (1.1). Every boosted list page
  // wraps its dynamic region in an element carrying hx-boost / hx-select /
  // hx-target / hx-swap — see patients_list.html etc. The server route,
  // URL, and returned data are unchanged; htmx just extracts the same
  // fragment out of the same full-page response instead of navigating.
  // ---------------------------------------------------------------
  if (window.htmx) {
