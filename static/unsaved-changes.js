...
(function () {
    ...
    if (btn) {
      btn.addEventListener("click", async () => {
        ...
        if (!ok) {
          updateSaveButton();
          window.VZToast.show(
            "Some changes couldn't be saved — please check your connection and try again. The items that failed are still highlighted.",
            "error"
          );
          return;
        }
        // 1.1: the fetch() above already persisted every change — no full
        // reload needed, the table already shows exactly what was saved.
        window.VZToast.show("Changes saved.", "success");
      });
    }
    updateSaveButton();
  });
  ...
  function showModal() {
    ...
    window.VZSpring.present(document.getElementById("unsavedModalOverlay"), true, {});
  ...
  function hideModal() {
    ...
    if (el) window.VZSpring.present(el, false, {});
    pendingAction = null;
  }
  ...
  async function proceed(save) {
    ...
    if (save) {
      ...
      if (!ok) {
        ...
        saveBtn.textContent = "Save & Continue";
        window.VZToast.show(
          "Some changes couldn't be saved — please check your connection and try again. You're still on this page and nothing else has been lost.",
          "error"
        );
