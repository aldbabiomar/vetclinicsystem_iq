...
def managed_data_dir():
    """Returns vetclinicsystemiq-data/ if this install has been switched onto
    ...
    env_dir = os.environ.get("VETCLINICSYSTEMIQ_DATA_DIR")
    ...
    # under vetclinicsystemiq-releases/, in which case the sibling data dir
    ...
    candidate = os.path.join(parent, "vetclinicsystemiq-data")
...
def _macos_launcher_path():
    ...
    return os.path.join(base, "Start VetClinicSystem.command")
...
TASK_NAME = "VetClinicSystemIQ Autostart"
...
def _windows_launcher_path():
    data_dir = managed_data_dir()
    base = data_dir if data_dir else BASE_DIR
    return os.path.join(base, "Start VetClinicSystem.bat")
...
def _windows_enable():
    ...
    launcher = _windows_launcher_path()
    if not os.path.isfile(launcher):
        return False, f"Could not find “Start VetClinicSystem IQ.bat” at {launcher} — can't set up automatic startup."

    task_ok, task_out = _windows_task_create()
    folder_ok, folder_err = _windows_startup_folder_enable(launcher)

    if task_ok:
        return True, ("VetClinicSystem IQ will now start automatically when this "
    ...
    if folder_ok:
        ...
        return True, ("VetClinicSystem IQ will now start automatically when you sign in. "
