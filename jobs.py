...
enough to need a visible progress bar: Backup Now, Restore, Insights,
Monthly/Yearly P&L, and Retention.
...
named phases like "Dumping database" or "Loading revenue by category"),
...
def start(step_labels, fn):
    ...
    def update(step_index, label=None, fraction=None):
        ...
        single step, e.g. turning "Restoring database" into "Restoring
        database (42/128 objects)" as real progress comes in from
        pg_restore, without that counter needing to be a step of its own.
        fraction (0..1), if given, overrides the current/total-steps
        percentage the client would otherwise compute — for a job that's
        conceptually one step but has a real internal count (e.g.
        rebuilding a report summary month by month), this lets the
        progress bar reflect that real count instead of just jumping from
        0% to 100%."""
        with _lock:
            if job_id in _jobs:
