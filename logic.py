...
def fmt_money(amount):
    """IQD has no practical decimal subdivision in everyday use — whole numbers, comma-separated."""
    if amount is None:
        return "\u2014"
...
def list_audit_sessions(db, limit=None, offset=0):
    """limit=None returns every session (unpaginated) — no other caller
    currently needs that, but kept as the default so this stays a
    drop-in replacement for the old unconditional-fetch behavior."""
    q = (
...
def count_audit_sessions(db):
    return db.execute("SELECT COUNT(*) c FROM audit_sessions").fetchone()["c"]
...
def _txn_qty_since_batch(db, cutoffs):
    ...
    round-trip per item. Returns {item_id: net_change_since_cutoff},
    omitting items with no net change."""
    if not cutoffs:
        return {}
...
def refresh_visit_billing_total(db, visit_id):
    """Recomputes and persists billing.total — the final, 250-rounded
    payable figure — after any change to this bill's lines, discount, or
    manual amount. Call this in the same transaction right after such a
    change, before commit. See the `total` column comment in
    schema_postgres.sql for why this is stored rather than re-derived."""
...
def inpatient_billing_summary(db, case_id):
    ...
    for r in rows:
        ...
        # that specific line's snapshot is NULL (e.g. the price_list item
        # had no sale_price set at the moment it was billed).
...
def refresh_inpatient_total(db, case_id):
    """Recomputes and persists inpatient_cases.total — the final,
    250-rounded payable figure — after any change to this case's
    procedures or discount. Call this in the same transaction right after
    such a change, before commit. See refresh_visit_billing_total()."""
...
def refresh_boarding_total(db, boarding_id):
    """Recomputes and persists boarding_sessions.billed_total — the final,
    250-rounded payable figure (distinct from `total`, the raw pre-
    rounding subtotal) — after the session is saved. Call this in the
    same transaction right after such a change, before commit. See
...
def followups(db, only_pending=False):
    ...
    """
    params = []
    # Moved from a Python `continue` filter to SQL — same rows either
    # way (v.followup_status != 'Pending' in Python is equivalent to
    # v.followup_status = 'Pending' in SQL's WHERE, including how each
    # treats a NULL status: both exclude it), just done by the database
    # instead of after fetching every row.
    if only_pending:
...
def followups_page(db, only_pending=False, limit=20, offset=0):
    ...
    dashboard's missed-items summary and today/tomorrow reminder counts
    (logic.missed_items(), logic.dashboard_counts()), which need every
    matching row to scan for "missed", not just one page of them.
...
def wellness_reminders_page(db, limit=20, offset=0):
    ...
    Deliberately doesn't support only_due=True: "due" depends on
    today's date at request time, not a stored column, so filtering by
    it can't move into SQL the way only_pending could for followups —
    and the one only_due=True caller (logic.dashboard_counts()) wants
    every matching row for its count, not one page, so it keeps calling
...
def _revenue_and_cogs_by_month(db, month=None):
    ...
    # the stored, already-250-rounded billing.total (kept in sync by
    # logic.refresh_visit_billing_total()) instead of re-deriving
    # subtotal*(1-discount%), which would drift from the receipt by up to
    # ~125 IQD now that bills round to the nearest 250 IQD note. A visit's
    # bill is always attributed to a single month (date_billed), so this
    # substitution changes nothing about *which* month gets the revenue.
    ...
    for r in db.execute(
        ...
        if r["billing_type"] != "Manual":
            # no line-level cost basis on a lump manual entry
            snapshot_rows = db.execute(
                "SELECT quantity, unit_cost FROM visit_billing_lines WHERE visit_id=?", (r["visit_id"],)
            ).fetchall()
            cogs_by_month[mth] += sum((l["unit_cost"] or 0) * l["quantity"] for l in snapshot_rows)
        revenue_by_month[mth] += r["total"] or 0

    # Reads the stored, already-250-rounded total (set at checkout — see
    # app.py's pos_checkout()) rather than re-deriving subtotal*(1-discount%),
    # which would drift from the receipt by up to ~125 IQD per sale now that
    # checkout rounds to the nearest 250 IQD note.
    ...
    # procedure was actually logged — but unlike visit billing, a single
    # case can span multiple months, so there's no one month to just read a
    # stored total for. Instead, each line's raw (pre-discount, pre-
    # rounding) amount is scaled so that all of one case's lines always sum
    # to exactly that case's stored, already-250-rounded total (kept in
    # sync by logic.refresh_inpatient_total()) — the actual amount ever
    # charged — rather than the raw pre-rounding sum, which can drift from
    # the receipt by up to ~125 IQD. Prefers the unit_price/unit_cost
    # snapshotted when the line was added; a NULL snapshot (that specific
    # price_list item had no sale_price/cost_price set at billing time)
    # falls back to the live Price List join, same reasoning as visit
    # billing above.
    ...
    lines_by_case = defaultdict(list)
    for r in ib_rows:
        unit_price = r["unit_price"] if r["unit_price"] is not None else (r["sale_price"] or 0)
    ...
    for case_id, case_lines in lines_by_case.items():
        raw_subtotal = sum(amt for _, amt, _ in case_lines)
        scale = (case_totals.get(case_id, 0) / raw_subtotal) if raw_subtotal else 0
        for mth, amt, cost in case_lines:
            if month and mth != month:
                continue
            revenue_by_month[mth] += amt * scale
            cogs_by_month[mth] += cost
    ...
    # Reads the stored, already-250-rounded billed_total (kept in sync by
    # logic.refresh_boarding_total()) rather than the raw pre-rounding
    # `total` column, which would drift from the receipt by up to ~125 IQD.
    boarding_where = " AND entry_date::text LIKE ?" if month else ""
    boarding_params = [month_like] if month else []
    for r in db.execute(
        "SELECT entry_date, billed_total FROM boarding_sessions WHERE billed_total IS NOT NULL" + boarding_where,
        boarding_params,
    ).fetchall():
        revenue_by_month[month_key(r["entry_date"])] += r["billed_total"]

    # Retail sale COGS: prefers the cost snapshotted at sale time
    # (sale_items.unit_cost); a NULL snapshot (that item had no
    # cost_price set at the moment it was sold) falls back to *current*
    # inventory cost as the best available approximation.
    ...
    # sale. refund_items doesn't link back to the specific sale_items row
    # it came from (only the item, quantity, and refunded price), so this
    # still uses *current* inventory cost to value the reversal — a
    # pre-existing approximation, not something this pass changes; fixing
    # it properly would mean adding a sale_item_id link to refund_items,
    # a bigger schema change than what was asked here.
...
def recompute_full_summary(db, on_progress=None):
    """
    ...
    shared cost/price value changes (which can still retroactively affect
    COGS/revenue for any billing/inpatient/sale row whose own snapshot is
    missing — see the fallback-to-live-lookup paths in
    _revenue_and_cogs_by_month — though every row written going forward
    carries its own frozen price/cost and is unaffected by later edits),
    plus as a manual admin "Rebuild" action.

    on_progress(done, total), if given, is called once for the initial
    full-history scan and then once per month written — a real month-by-
    month count, not a simulated one, since that scan (reading every
    billing/sale/inpatient/refund row ever recorded) is the genuinely slow
    part on a clinic with years of history, and the per-month writes that
    follow are proportionally quick. Does not commit.
    """
    if on_progress:
        on_progress(0, 0)  # total unknown yet — still scanning full history
    revenue_by_month, cogs_by_month = _revenue_and_cogs_by_month(db)
    months = sorted(set(revenue_by_month) | set(cogs_by_month))
    now_str = datetime.now().isoformat(timespec="seconds")
...
def revenue_by_category(db, months_back=12):
    ...
    rows = db.execute(
        """
        WITH auto_lines AS (
          -- Each line's raw amount is scaled so a bill's lines always sum
          -- to exactly that bill's stored, already-250-rounded total
          -- (billing.total, kept in sync by
          -- logic.refresh_visit_billing_total()) instead of the raw
          -- pre-rounding sum, which would drift from the receipt by up to
        ...
        manual_lines AS (
          -- A Manual bill is always a single "Veterinary Services" line, so
          -- its stored rounded total already IS this line's amount — no
          -- split needed.
        ...
        inpatient_lines AS (
          -- Same proportional-scaling technique as auto_lines above,
          -- against inpatient_cases.total (kept in sync by
        ...
        boarding_lines AS (
          -- Reads the stored, already-250-rounded billed_total (kept in
          -- sync by logic.refresh_boarding_total()) rather than the raw
          -- pre-rounding `total` column.
          SELECT to_char(bs.entry_date, 'YYYY-MM') AS month, 'Boarding' AS category, COALESCE(bs.billed_total,0) AS amount
          FROM boarding_sessions bs
          WHERE bs.billed_total IS NOT NULL AND bs.entry_date >= ?
        ),
        refund_lines AS (
...
def patient_outpatient_visits(db, patient_id, cases=None, order="DESC"):
    """
    A patient's visits, minus any visit_type='Inpatient' row that's just the
    admitting encounter for a stay already covered by inpatient_cases (same
    date) -- staff mark the triggering visit as type "Inpatient" as part of
    admitting someone, but that encounter is already fully represented by
    the inpatient_cases row itself. Without this, the exact same visit
    (same complaint/exam/treatment) shows up twice everywhere patient
    history is shown: once as a "visit," once as an "inpatient stay."
    visit_type is a plain editable field, not database-enforced against
    inpatient_cases, so this only drops a visit when there's an actual
    matching admission on the same date -- a visit someone marked
    "Inpatient" without ever creating the case still shows up, rather than
    ...
    return [v for v in visits if not (v["visit_type"] == "Inpatient" and v["date"] in admission_dates)]
...
def generate_slots(db):
    ...
    try:
        ...
        # (Appointments, New Visit, Grooming, Inpatient's vet picker).
...
def vet_users(db):
    """
    Active staff who can be assigned as the attending vet — anyone whose
    ...
    return db.execute(
        ...
        "WHERE r.is_vet_role=true AND u.active=true ORDER BY u.full_name"
    ).fetchall()
...
def record_consignment_shrinkage(db, item_id, distributor_id, quantity, reason, liable_party,
    ...
    POS checkout closes for a sale (two concurrent write-offs could
    ...
    # closed (same reasoning as the POS checkout fix) rather than let
    # `quantity > None` either silently pass or raise a TypeError.
...
def consignment_balance(db, distributor_id):
    """
    The payable balance for one distributor, as of right now. Mirrors the
    scan-then-sum shape of _revenue_and_cogs_by_month on purpose — same
    ...
    sale_items row it came from (only item_id + quantity), so — same
    acknowledged limitation as _revenue_and_cogs_by_month's refund
    handling — it values the reversal at *current* inventory_list.cost_price
    rather than the original sale's cost. In practice this only matters
    if that item's cost_price changed between the original sale and the
    refund, which is a narrow window for something a clinic would return.
...
def consignment_distributors_overview(db):
    """
    One row per distributor with >=1 Consignment item (§6.1 Overview):
    shelf stock (units + value at their cost), amount owed right now,
    last settlement date, units sold this month. Calls
...
def consignment_sales_by_distributor(db, distributor_id=None, date_from=None, date_to=None):
    """
    §6.6 Sales by Distributor report. Uses sale_items.unit_cost — the
...
def consignment_item_locked(db, item_id):
    ...
    distributor_id becomes uneditable in the UI (§3.1: an item never
...
# ---------------------------------------------------------------------------
# Cash Register — a single unified daily ledger across every place money
# actually changes hands: POS sales, Visit/Inpatient/Boarding payments, and
# refunds (negative, so they net out automatically). Distinct from every
# other billing table in this module, which tracks what's *owed*, not what's
# been *collected* — that distinction is the whole point of this feature:
# comparing what the system says came in against what's physically in the
# till, to catch poor documentation or leaking/stolen cash early. See
# app.py's cash_register_page for the routes that use this.
#
