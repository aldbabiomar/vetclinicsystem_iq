...
def apply_incremental_migrations():
    """One connection, but each statement in its own savepoint — a single
    failing statement (e.g. the duplicate-owner-phone unique index, if a
    database already has a violating pair) used to abort the whole
    transaction, silently skipping every statement after it, on every
    single launch, forever. Now a failure is isolated to that one
    statement; the rest still apply. See ORPHANED_RECORDS_AUDIT.md F-22."""
    ...
    elif INCREMENTAL_SCHEMA_STATEMENTS:
