...
- **Arabic.** The whole app now runs in Arabic — every page, every message,
  ...
  digits, currency reads د.ع, and the layout flips right-to-left.
...
- **A wrong password locked an account for the wrong length of time.** The
  lockout got longer with the number of failed attempts, but pausing between
  attempts reset it — so a steady stream of guesses was treated more gently
...
### Fixed
- **A malformed date in a page's web address no longer shows an error
  page.** Hand-editing the `?date=` part of the address on Visits or
  Refunds to something that started with a real date but had extra
  characters after it produced a blank error page; it now tells you the
  date wasn't valid and shows all dates, the same as any other bad date.
