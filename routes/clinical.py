...
from core import BadDate, BadNumber, BadPhone, PER_PAGE, clean_date, cleanup_amount_error, discount_percent_error, flash_cash_denomination_warning, get_db, get_page, has_negative, normalize_phone, page_count, page_offset, parse_int, parse_money, required_field
...
def _create_inpatient_case(db, patient_id, visit_id, complaint, admission_date, weight_kg=None, bcs=None):
    ...
    # afterwards -- put to the clinic owner explicitly and accepted, because
    # the alternative lets a large open stay be discounted after the fact.
...
def visit_edit(visit_id):
    ...
    if request.method == "POST":
        ...
        if conflict:
            ...
            return redirect(url_for("clinical.visit_edit", visit_id=visit_id))
        wellness_needed = f.get("wellness_needed", "N")
        grooming_needed = f.get("grooming_needed", "N")
        grooming_services = ",".join(f.getlist("grooming_services")) if grooming_needed == "Y" else None
...
def visit_billing_save(visit_id):
    ...
    if billing_type == "Automatic":
        ...
        for pid in f.getlist("price_id"):
            ...
            priced_lines.append({
                ...
                # Snapshotted for the same reason unit_price is, one line up:
                # a later Price List edit must not change what this bill
                # already charged, or what a refund against it pays back.
        ...
        # non-discountable item on a member's bill and break their billing
        # outright the moment the card went live.
    ...
    if existing:
        ...
        # From the cart being SAVED, not the bill as it stands — this check
        # is about what the bill is becoming. A Manual bill is discountable
        # in full (A4).
        ...
        if paid_row["s"] > new_total:
            flash(_("That change would leave %(fmt_money)s paid against a %(fmt_money2)s IQD bill. Process a service refund for the difference first.", fmt_money=logic.fmt_money(paid_row['s']), fmt_money2=logic.fmt_money(new_total)), "error")
            return redisplay()
    ...
    # list and deliberately NOT in the DO UPDATE clause below: every later
    # save of this bill goes through the UPDATE branch, and listing them
    # there would re-stamp the snapshot on every edit -- silently applying a
    # card the customer did not hold when the bill was raised, or wiping one
    # they did.
...
def visit_discount_save(visit_id):
    ...
    # uses owner_rewards_remove_discount() below, which can only ever remove.
...
def visit_payment_add(visit_id):
    ...
    if amount > balance + 1e-9:
        flash(_("That's more than the remaining balance of %(fmt_money)s IQD on this visit.", fmt_money=logic.fmt_money(balance)), "error")
        return redisplay()
    try:
...
def boarding_new():
    ...
    if not patient_id:
        flash(_("Pick a patient first."), "error")
...
def boarding_payment(boarding_id):
    ...
    _unused, _unused, balance, _unused, _unused = logic.compute_bill_totals(
        ...
        discountable_subtotal=summary["discountable_subtotal"])
    if amount > balance + 1e-9:
        flash(_("That's more than the remaining balance of %(fmt_money)s IQD on this stay.", fmt_money=logic.fmt_money(balance)), "error")
        return redisplay()
    cur = db.execute(
...
def inpatient_billing_add(case_id):
    ...
    # Scoped to STAFF discounts, same reasoning as visit_billing_save().
    if (existing_case and (existing_case["discount_percent"] or 0) > 0
            and existing_case["discount_source"] == "staff"):
        blocked = logic.non_discountable_line_names(db, price_ids)
        if blocked:
            flash(_("Can't add — this case has a %(discount_percent)s%% discount applied, but includes item(s) marked as not discountable: %(join)s. Remove the discount first, or leave these items off this bill.", discount_percent=f"{existing_case['discount_percent']:.0f}", join=', '.join(blocked)), "error")
            return redirect(url_for("clinical.inpatient_detail", case_id=case_id))
...
def inpatient_billing_delete(case_id, line_id):
    ...
    if summary["paid"] > remaining_total:
        flash(_("Removing this line would leave %(fmt_money)s paid against a %(fmt_money2)s IQD bill. Process a service refund for the difference first.", fmt_money=logic.fmt_money(summary['paid']), fmt_money2=logic.fmt_money(remaining_total)), "error")
        return redirect(url_for("clinical.inpatient_detail", case_id=case_id))
...
def inpatient_payment_add(case_id):
    ...
    balance = summary["balance"]
    if amount > balance + 1e-9:
        flash(_("That's more than the remaining balance of %(fmt_money)s IQD on this case.", fmt_money=logic.fmt_money(balance)), "error")
        return redisplay()
    try:
        cleanup_amount = parse_money(f.get("cleanup_amount")) or 0
    except BadNumber:
...
# Appointments
# ---------------------------------------------------------------------------
def _appointments_page_context(db, week_anchor, selected_day, show_past=False):
...
def appointments_page():
    ...
    # below never fired and "" reached Postgres as a date parameter, which is
    # an unhandled InvalidDatetimeFormat, i.e. a 500 on a page reachable by
    ...
    return render_template("appointments.html", **_appointments_page_context(db, week_anchor, selected_day, show_past))
