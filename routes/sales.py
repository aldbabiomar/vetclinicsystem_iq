...
def _record_sale(db, lines, *, subtotal, discount_percent, total, cleanup_amount,
    ...
    for iid, qty, price, line_total, unit_cost, distributor_id, discountable in lines:
        db.execute(
            ...
            "VALUES (?,?,?,?,?,?,?,?)",
            (sale_id, iid, qty, price, round(line_total, 2), unit_cost, distributor_id, discountable))
        db.execute(
...
def pos_checkout():
    ...
    # it and the walk-in path is unchanged, so owner_id is NULL on most
    # sales by design. It is filled in when a rewards card is presented.
    ...
    # snapshot — blocking the sale here would break POS for every member the
    # moment a non-discountable item entered the cart.
    ...
    total = money.payable_total(
        logic.discounted_raw_total(subtotal, discountable_subtotal, discount_percent),
        discount_percent)
    try:
