...
def resolve_restorable_backup(db, source_file):
    ...
    source. Previously any user with manage_settings could point the
    ...
    and the only check on submit was "does this path exist and end in
    ...
    process control, not this fix's job; this closes the "arbitrary
    path" hole while leaving restore a single-admin action, as asked.
...
def _run_backup_locked(db, dest_dir=None, retention=None, triggered_by=None, on_progress=None):
    ...
    retention) — the same three-argument shape jobs.py's update() uses,
    so a caller running this inside jobs.start() can just pass that
    straight through.
    """
