...
def _discount_row(subtotal, discount_percent, discount_source, pre_cleanup_total, suffix=""):
    ...
    return [f"{label} ({discount_percent:.0f}%)",
            f"-{subtotal - pre_cleanup_total:,.0f}{suffix}"]
...
def X(v):
    ...
    otherwise break the parser and crash the whole export. Every place in
    this file that interpolates a database/user-supplied value into a
    Paragraph string should route it through this first. Static markup we
    write ourselves (e.g. "<b>...</b>") is intentionally NOT passed
    through this — only the variable content is.
    """
    if v is None:
        return ""
...
def export_patient_file(db, patient_id):
    ...
    cases = logic.patient_inpatient_cases(db, patient_id, order="ASC")
    visits = logic.patient_outpatient_visits(db, patient_id, cases=cases, order="ASC")
...
def export_sale_receipt(db, sale_id):
    ...
    data = [["Item", "Unit Price (IQD)", "Qty", "Line Total (IQD)"]]
    for l in lines:
        data.append([l["name"], f"{l['unit_price']:,.0f}", f"{l['quantity']:g}", f"{l['line_total']:,.0f}"])
    ...
    _drow = _discount_row(sale["subtotal"], sale["discount_percent"], sale["discount_source"],
                          sale["total"] + (sale["cleanup_amount"] or 0), " IQD")
    if _drow:
...
def export_visit_pdf(db, visit_id):
    ...
    if summary["lines"]:
        data = [["Item", "Price (IQD)"]]
        for l in summary["lines"]:
            # Plain Table cell — drawn literally, no escaping needed.
            data.append([l["name"], f"{l['price']:,.0f}"])
        story.append(_section_table(data, [120 * mm, 45 * mm]))
        story.append(Spacer(1, 6))
    bill_rows = [["Subtotal", f"{summary['subtotal']:,.0f} IQD"]]
    _drow = _discount_row(summary["subtotal"], summary["discount_percent"],
                          summary.get("discount_source"), summary["pre_cleanup_total"], " IQD")
    if _drow:
...
def export_inpatient_pdf(db, case_id):
    ...
    story.append(Paragraph("Billing", ss["H2"]))
    if summary["lines"]:
        data = [["Procedure", "Qty", "Unit Price (IQD)", "Line Total (IQD)"]]
        for l in summary["lines"]:
            # Plain Table cell — drawn literally, no escaping needed.
...
def export_consignment_settlement_pdf(db, settlement_id):
    ...
    rows = [
        ["Amount Owed", f"{s['amount_owed']:,.0f} IQD"],
        ["Amount Paid", f"{s['amount_paid']:,.0f} IQD"],
        ["Payment Method", s["payment_method"] or "\u2014"],
    ]
    if residual > 0:
        rows.append(["Carried Forward to Next Settlement", f"{residual:,.0f} IQD"])
...
def export_distributor_ledger(db, distributor_id):
    ...
    story = [
        ...
        Spacer(1, 10),
        Paragraph(
            f"<b>Total Billed:</b> {ledger['total_billed']:,.0f} IQD &nbsp;&nbsp; "
            f"<b>Total Paid:</b> {ledger['total_paid']:,.0f} IQD &nbsp;&nbsp; "
            f"<b>Outstanding:</b> {ledger['total_outstanding']:,.0f} IQD",
            ss["Body"],
        ),
        Spacer(1, 14),
    ]
    ...
    for bill in ledger["bills"]:
        ...
        data = [["Payment Date", "Amount (IQD)", "Method", "Notes"]]
